...
describe("addition operator (+)", () => {
  ...
  it("a + b emits ActionAdd2 (0x47 or 0x64)", () => {
    const bytes = compileAS2("var a = 1; var b = 2; a + b;");
    // Some AVM1 compilers emit 0x47 (ActionAdd2 extended) or 0x64 (ActionAdd2)
    expect(containsByte(bytes, 0x47) || containsByte(bytes, 0x64)).toBe(true);
  });
});
...
describe("modulo operator (%)", () => {
  ...
  it("a % b emits ActionModulo (0x3f or 0x63)", () => {
    const bytes = compileAS2("var a = 7; var b = 3; a % b;");
    // AVM1 ActionModulo is either 0x3f or 0x63 depending on compiler version
    expect(containsByte(bytes, 0x3f) || containsByte(bytes, 0x63)).toBe(true);
  });
});
...
describe("operator precedence (multiply before add)", () => {
  ...
  it("2 + 3 * 4 emits both ActionMultiply (0x0c) and ActionAdd2 (0x47 or 0x64)", () => {
    const bytes = compileAS2("2 + 3 * 4;");
    expect(containsByte(bytes, 0x0c)).toBe(true); // ActionMultiply
    expect(containsByte(bytes, 0x47) || containsByte(bytes, 0x64)).toBe(true); // ActionAdd2
  });
});
