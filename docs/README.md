...
| 36 | [Vector Merge Model](./36-vector-merge-model.md) | Authentic Flash 8 merge-drawing ("shape soup" fill0/fill1): same-color union / different-color cut / line-splits-fill / segment selection / true subtraction. The curve-preserving half-edge planar kernel (engine/planar/) + phased plan P0–P5 + Ruffle/stage oracle verification |
| 37 | [Collaboration (P2P multiplayer)](./37-collab.md) | Optional opt-in real-time collaboration. P0: the `@flash/collab` Yjs derived-binding — faithful `FlashDocument`↔`Y.Doc` projection (per-field scalar merge, atomic geometry, char-level `asClasses`), structural-sharing outbound diff + origin-filtered inbound `replaceDoc`. Property-tested as an identity; default OFF, no networking yet |
